#!/usr/bin/env node
/**
 * dsh-companion: the execution body shared by both hosts of the dsh plugin.
 *
...
function printUsage() {
  process.stdout.write(
    [
      ...
      "  transfer --source <transcript.jsonl> [--cwd <dir>] [--json]",
...
const CLAUDE_RECORD_TYPES = new Set(["assistant", "mode", "summary", "system", "user"]);

function isClaudeSessionRecord(record) {
  if (!record || typeof record !== "object") {
    return false;
  }
  if (record.message && typeof record.message.role === "string") {
    return true;
  }
  return CLAUDE_RECORD_TYPES.has(String(record.type || ""));
}

function isCodexSessionRecord(record) {
  if (!record || typeof record !== "object") {
    return false;
  }
...
/** Message content is a string or a list of blocks; only text blocks carry text. */
...
function readCodexTranscript(records) {
  ...
  for (const record of records.slice(1)) {
    ...
    const body = messageText(record.content || (record.payload && record.payload.content)).trim();
    if (body) {
      turns.push(role.toUpperCase() + ": " + body);
    }
  }
  return {
    host: "Codex",
    cwd: meta.cwd || meta.payload?.cwd || null,
    turns: turns
  };
}

function readClaudeTranscript(records) {
  const turns = [];
  let cwd = null;
  for (const record of records) {
    ...
    // A sidechain is a subagent conversation, not the conversation being handed over.
    if (record.isSidechain === true) {
      continue;
    }
    const role = (record.message && record.message.role)
      || (record.type === "user" || record.type === "assistant" ? record.type : null);
    if (role !== "user" && role !== "assistant") {
      continue;
    }
    const content = record.message && record.message.content !== undefined
      ? record.message.content
      : record.content;
    const body = messageText(content).trim();
    if (body) {
      turns.push(role.toUpperCase() + ": " + body);
    }
  }
  return { host: "Claude Code", cwd: cwd, turns: turns };
}

/**
 * Read one transcript into a hand-over body. Two shapes are accepted - a Codex
 * rollout and a Claude Code session - and the file itself decides which one it
 * is, so `transfer` stays a single command for both hosts.
 */
function extractTranscript(sourcePath) {
  if (sourcePath.endsWith(".zst") || sourcePath.endsWith(".jsonl.zst")) {
    throw new Error("Compressed transcripts are not supported. Decompress the .jsonl.zst file first.");
  }
  ...
  const parsed = isClaudeSessionRecord(first)
    ? readClaudeTranscript(records)
    : isCodexSessionRecord(first)
      ? readCodexTranscript(records)
      : null;
  if (parsed === null) {
    throw new Error("The transcript is neither a Codex rollout nor a Claude Code session: " + sourcePath);
  }
  if (parsed.turns.length === 0) {
    throw new Error("The transcript holds no user or assistant text: " + sourcePath);
  }
  return { host: parsed.host, cwd: parsed.cwd, transcript: parsed.turns.join("\n\n") };
}
...
async function handleTransfer(argv) {
  ...
  if (!options.source) {
    throw new Error("transfer requires --source <transcript.jsonl>; the plugin never guesses the current conversation.");
  }
  const cwd = resolveCommandCwd(options);
  const workspaceRoot = resolveCommandWorkspace(options);
  const sourcePath = path.resolve(cwd, String(options.source));
  const { host, cwd: sourceCwd, transcript } = extractTranscript(sourcePath);
  ensureDshReady(cwd);
  ...
  const prompt = [
    "The following " + host + " conversation is being handed over to you. Continue the work from here.",
    "",
    "Source transcript: " + sourcePath,
    sourceCwd ? host + " working directory: " + sourceCwd : null,
  ...
  const job = createJob({
    ...
    title: "DSH Transfer",
    workspaceRoot: workspaceRoot,
    summary: shorten("Transferred " + host + " conversation from " + sourcePath)
  });
  const request = {
    kind: "task",
    jobId: job.id,
  ...
  if (options.json) {
    ...
    outputResult(
      { ...execution.payload, jobId: job.id, host: host, sourcePath: sourcePath, truncated: truncated },
      true
    );
  } else {
    outputResult(
      renderTransferResult({ sessionId: execution.sessionId, host: host, sourcePath: sourcePath, truncated: truncated, maxBytes: budget }),
      false
    );
  }
  if (execution.exitStatus !== 0) {
